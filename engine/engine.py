...
class MemoryEngine:
    ...
    def new_event(self, event: Event, *, store: bool = True,
        ...
        if encoded and store:
            with self.tel.stage("encode"):
                ...
                episode_id = rec.id
                bt = event.features.get("body_text") or \
                     event.features.get("subject_norm")
                if bt:
                    self.store.fts_upsert(str(rec.id),
                        subj := (event.features.get("subject_norm") or "")
                        + " " + bt)
    ...
    def activate(self, cue: dict, k: int = 8) -> list[dict]:
        """Fused fuzzy recall. Cue channels (all optional):
          text:      -> lexical FTS5 + semantic embedding (if embedder bound)
          persons:   -> person boost via stored person lists
          stream:    -> restrict candidate set
          tau_now / tau_target: recency channel per H15 ladder
        Returns [{episode_id, score_kind...}] ranked by reciprocal-rank fusion.
        """
        with self.tel.stage("activate"):
            channels: list[list[str]] = []
            weights: list[float] = []

            if cue.get("embedding") and len(self.vec_index) > 0:
                hits = self.vec_index.search(cue["embedding"], k=k * 3)
            ...
            # recency channel: strongest for single-stream cues
            eps = self.store.iter_episodes()
            if cue.get("stream"):
                eps = [e for e in eps if e.stream == cue["stream"]]
            now = cue.get("tau_now", max((e.tau for e in eps), default=0.0))
            recency = sorted(
                eps,
                key=lambda e: -(e.strength if math.isinf(e.strength) else 0.0)
                * math.exp(-((now - e.tau) / max(e.strength, 1e-9)))
                if not math.isinf(e.strength) else float("inf"))
            rec_ids = [str(e.id) for e in recency[: k * 3]]
            if rec_ids:
            ...
            for rank, eid in enumerate(fused, 1):
                ep = next((x for x in eps if str(x.id) == eid), None)
                pboost = 1.3 if persons & set(ep.persons or []) else 1.0 if ep else 1.0
                out.append({"episode_id": eid,
                            "rrf_score": round(
                                sum(w / (60 + i + 1)
                                    for ch, w in zip(channels, weights)
                                    for i, x in enumerate(ch) if x == eid) * pboost, 4),
                            "person_boost": pboost})
    ...
    # -----------------------------------------------------------------------
    def prune(self, tau_now: float | None = None) -> PruneReport:
